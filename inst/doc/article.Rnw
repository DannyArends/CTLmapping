...
\begin{figure}[ht]
  \centerline{\includegraphics[height=7cm]{qcl.pdf}}
  \vspace{-15pt}
  \caption{{\scriptsize QCL mapping: We start by looking at a single genetic marker and dividing our populaiton into two groups.
...
\begin{figure}[ht]
  \centerline{\includegraphics[height=6cm]{qcl_n.pdf}}
  \vspace{-15pt}
...
op <- par(cex=1)
@ 
  \caption{{\scriptsize QCL scores obtained during permutations (A. Thaliana) additional information obtained by QCL mapping, 
...
op <- par(cex=1)
@
  \caption{{\scriptsize QCL profile (A. Thaliana, X3.Hydroxypropyl) transformed to LOD scores compared to the QTL profile of the same phenotype, We see an 
...
op <- par(cex=0.5)
QCLasLOD(ath.result[[1]], ath.result[[1]]$qtl)
op <- par(cex=1)
@ 
  \caption{{\scriptsize QCL profile (A. Thaliana, X3.Hydroxypropyl) transformed to LOD scores. Shown is the likelyhood contribution of each of 
  the 24 metabolites to the QCL profile.}}
...
@ 
  \caption{{\scriptsize Phenotypes x Phenotypes matrix (A. Thaliana) additional information obtained by QCL mapping, 
  which can be used to reconstruct the underlying genetic architecture}}
  \label{fig:p2pmatrix}
\end{figure}
