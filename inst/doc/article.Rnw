...
op <- par(cex=0.5)
QCLasLOD(qcl_result[[1]], qcl_result[[1]]$qtl)
op <- par(cex=1)
@ 
...
plot(qcl_result, against="markers", do.grid=TRUE, grid.col="white")
@ 
...
par(mar=c(3,8,1.1,1.1))
QTLimage(qcl_result, do.grid=TRUE, grid.col="white")
@
...
par(mar=c(3,8,1.1,1.1))
plot(qcl_result, against="phenotypes", do.grid=TRUE, grid.col="white")
@ 
