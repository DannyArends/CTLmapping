...
\author
{Danny Arends,$^{1\ast,2}$ Ritsert C. Jansen,$^{1}$\\
\\
...
<<echo=FALSE,results=hide>>=
  ...
  qcl_result <- QCLscan(ath.metab$genotypes, ath.metab$phenotypes)
@
...
or other experimental crosses. QCL mapping allows association of phenotype correlation 
...
complex traits. The method also provides a vital link in regulatory network reconstruction.
\end{sciabstract}
...
individuals carrying the other genotype. We merge information coming from QCL and QTL to reveal 
...
\emph{\sffamily Single phenotype QCL}
...
See figures XX, and XX showing the score distribution in permuted data.

\emph{\sffamily Creating single trait QCL profiles}

After permutation single trait QCL profiles can be obtained by setting a qcl significance 
threshold. This threshold defines when a change in correlation is significant, and at 
each marker we can plot the count of number the other phenotypes that show a significant 
difference in correlation. Overplotting chromosome, QTL information, and phenotype location 
(if applicable) produces the single QCL plots seen in figures XX, and XX.

\emph{\sffamily Step 3 Network scan for QCL}

If we find any significant QCL information we can try to reconstruct the interaction 
network of the phenotype. We first do a single phenotype QCL scan (step 2), then 
using a permutation determin a qcl threshold, and select phenotypes which show a 
significant change in QCL.

Two things can occur: 
1) No genes show a significant change in QCL, which means we 
cannot find any interaction partners for this phenotype, or 
2) This genes has an QCL with other genes, then the algorithm adds 
these genes onto a list to scan and repeats the single phenotype 
QCL scan (step 2) for these phenotypes (not allowing for duplicate 
scans). This is repeated until no more significant QCL are found or 
a user defined search depth is reached.
...
to show a cis-QTL, this is because a QTL can induce a QCL, because variation of expression 
in the phenotype is causing other phenotypes to become uncorrelated, when assigned to 
groups determined by the QTL marker. This phenotype (or phenotypes) introduced genetic 
variation, which is seen also (and more pronounced) in a QTL scan. 

\emph{\sffamily Visualizing the Phenotypes x Phenotypes relations}

The additional information obtained by QCL mapping is summarized in the 
Phenotypes x Phenotypes matrix. This shows the summed evidence of interaction 
between the phenotypes showing QCL transmitted through all the markers and the phenotype scanned.
...
\emph{\sffamily Example 1. A. Thaliana}

QCL mapping was performed on 24 characterized metabolities measured in Arabidopsis 
Thaliana. A significance threshold for QCL mapping was determined by using 1000 
permutations, the outcomes can be seen in table XX. We opted for the stringent 
threshold at P < 0.001 to use as 'significant' QCL score for further analysis.

Overview plots of the results of the QCL mapping in Arabidopsis can be found in 
...
but now represented as an interaction network is seen in figure XX
\begin{table}[ht]
\begin{tabular}{ l | c c }
P-value & QCL threshold Arabidopsis & QCL threshold Saccharomyces\\
\hline
5 \% & 0.220 & 0.436\\
1 \% & 0.294 & 0.491\\
0.1 \% & 0.375 & 0.556\\
\end{tabular}
\caption{QCL thresholds obtained after 1000 permutations}
\label{permutations}
\end{table}

\emph{\sffamily Example 1. S. Cerevisiae}

QCL mapping was performed on 
...
Many genetical genomics screens have been carried out to map differential 
...
mapping Mendelian traits. However results on complex traits have been less 
then satisfactory with only small amounts of heritability explained by 
individuals loci. 
QCL mapping however 'borrows' information from each phenotype in the analysis set, 
and enables extraction of phenotype to marker to phenotype networks. 
...
architecture. 
It can provides better insight in the missing of heritability of variation, we 
...
\begin{figure}[ht]
  ...
  and P2. In our model a genetic pertubation is causing P1 to show a QTL at a certain marker. We expect variation 
...
\begin{figure}
  \centering
<<singletrait, fig=TRUE, echo=FALSE, height=3.5>>=
plot(qcl_result, 2, do.legend=FALSE)
@ 
  \caption{Single trait QCL profile (A. Thaliana) shows the number of phenotypes that have a significant 
  QCL with the scanned phenotype at different QCL significance thresholds.\label{fig:singletrait}}
\end{figure}

\begin{figure}[ht]
  \centering
<<p2mmatrix, fig=TRUE, echo=FALSE, height=3.5>>=
par(mar=c(3,8,1.1,1.1))
image(qcl_result, 0.5, against="markers",do.grid=FALSE)
@ 
...
\begin{figure}[ht]
  \centering
<<p2pmatrix, fig=TRUE, echo=FALSE, height=3.5>>=
par(mar=c(3,8,1.1,1.1))
...
\begin{figure}[ht]
  \centerline{\includegraphics{AraPerm.pdf}}
  \caption{Histogram of scores obtained durting permutation of the QCL mapping methodology}
  \label{AraPerm}
\end{figure}
