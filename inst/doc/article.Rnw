...
\begin{center}
\vspace*{1in}
\textbf{\Large QCL mapping in experimental crosses}
...
\textbf{\sffamily Abstract}: Quanitative correlation locus (QCL) mapping is 
a novel approach to detect genetic regulation of phenotypes in recombinant 
inbred line populations (RIL)
\vfill
