...
?ndividuals?allows for estimation of the underlying null-distribution of QCL scores. 

\emph{\sffamily Step 2b Create single trait QCL profiles}

Single trait QCL profiles can be obtained by setting a significance threshold. 
