...
\begin{center}
\vspace*{1in}
\textbf{\Large QCL: Added value of differential correlation }
...
\textbf{\sffamily Algorithm: QCL mapping}
...
@ 
\caption{Single trait QCL profile (A. Thaliana).\label{fig:singletrait}}
\end{figure}
...
Global overview of QCL scan after using a full scan for QCL
<<echo=FALSE>>=
  qcl_result
@ 
Single results from a QCL mapping, shows the pattern at the highest threshold.
<<echo=FALSE>>=
  qcl_result[[2]]
@ 
Phenotype x Marker image plot, this is 'comparable' to a QTL scan on mutliple traits.
...
@ 
\caption{Phenotypes x Markers matrix (A. Thaliana).\label{fig:p2mmatrix}}
\end{figure}

Phenotype x Phenotype image plot, the new network information obtained by QCL mapping, 
...
@ 
\caption{Phenotypes x Phenotypes matrix (A. Thaliana).\label{fig:p2pmatrix}}
\end{figure}
