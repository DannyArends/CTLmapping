...
<<echo=FALSE,results=hide>>=
  ...
  cat("QTL scan\n")
  qtl_result <- QTLscan(ath.metab$genotypes, ath.metab$phenotypes)
  cat("QCL permutation\n")
  qcl_perms1 <- QCLpermute(ath.metab$genotypes, ath.metab$phenotypes, 1:3, 200, n.cores = 3)
  qcl_perms2 <- QCLpermute(ath.metab$genotypes, ath.metab$phenotypes, 4:24, 50, n.cores = 3)
  qcl_perms <- c(qcl_perms1,qcl_perms2)
  #qcl_perms <- vector("list",24)
  #idx <- 1
  #for(p in 1:24){
  #  qcl_perms[[idx]] <- read.QCLpermute("permutations", p, 300,TRUE)
  #  idx <- idx+1
  #}
@
...
op <- par(cex=0.5)
plot(c(0,0.8),c(0,400),type='n',main="QCL scores permutations",ylab="Frequency",xlab="Difference in correlation^2")
