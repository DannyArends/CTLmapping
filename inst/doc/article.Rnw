...
\title{CTL: Added value of differential correlation} 
...
\begin{sciabstract}
  In this paper we present Quanitative Correlation Locus (CTL) mapping, a novel approach 
...
\section*{Introduction}
  ...
  recognized and this led to the development of this algorithm to perform CTL analysis in 
  ...
  This is what CTL mapping exploits, the hypothesis is that although B doesn't show a QTL, 
  ...
  CTL thus maps not differential co-expression but differential correlation to genetic 
  ...
  We merge information coming from CTL and QTL to reveal underlying genetic architecture, 
  which would remain hidden when using only classical QTL information.

  We urge all QTL researchers to combine the new CTL method with existing QTL analysis to expose 
  transcriptional networks involved in the regulation of complex phenotypes. For this we 
  provide an R package with a reference implementation of the CTL algorithm.
\bigskip 

\section*{Methodology}
\nopagebreak

\emph{\sffamily CTL mapping}\\
  As mentioned in the introduction CTL mapping uses buffering to detect interaction partners. 
  ...
  phenotypes to show differential correlation with the inducing phenotype, because CTL relies on 
  correlation changes as opposed to differences in phenotype mean CTL is unaffected by (p2).
...
\emph{\sffamily Single phenotype CTL mapping}\\
  ...
  and B, and calculate the CTL matrix by taking the squared difference between matrix A 
  and B.

\emph{\sffamily Significance testing}\\
  To compare to QTL mapping we need to convert the CTLscores to LOD scores. Significance 
  of CTL were assessed by using permutations, in each round the link between genotype and 
  ...
  the underlying null-distribution of CTL scores. We frequently observe CTL scores in 
  real data which is higher than any CTL score obtained during permutation. When this 
...
\emph{\sffamily Creating single trait CTL profiles}\\
  After permutation single trait CTL profiles can be obtained by transforming the CTL scores 
  into LOD scores. For this we use a look-up table of scores obtained during permutation, we 
  assess the likelyhood and take the $-log10(P.value)$ of every CTL score see Fig. 
  \ref{fig:singletraitLOD}. This is done 1) to compare QTL profiles with summarized CTL profiles 
  (an example in Fig. \ref{fig:singletrait}), and 2) Show the contribution of each phenotype 
  to the total likelyhood.

\emph{\sffamily Visualizing the Phenotypes x Phenotypes CTL relations}\\
  In Fig. \ref{fig:singletraitLOD} we see the additional information obtained by CTL mapping summarized 
...
\emph{\sffamily Visualizing the Phenotypes x Marker relations}\\
  When plotting multiple CTL profiles as a heatmap, CTLs are seen mapping to the 
  same genetic location. At these locations, regulation is taking place. We expect 
  to find one (or more) of the genes to show a cis-QTL, this is because a CTL can 
...
\emph{\sffamily Network reconstruction}\\
  ...
  We show two examples in which we reconstruct the genetic architecture using CTL mapping. 
...
\nopagebreak
  ...
  CTL mapping was performed on 24 characterized metabolities measured in Arabidopsis 
  Thaliana. A significance threshold for CTL mapping was determined for each trait 
  by using 1000 permutations, the outcomes can be seen in Fig \ref{fig:histPermAra}. 
  We used a very stringent CTL threshold at P < 0.001 to use as 'significant' CTL 
  ...
  \emph{\sffamily Single trait CTL}\\
  The 24 CTL profiles were compared to the QTL profiles previously determined. We observed 3 cases:\\
  1) Additional CTL found, where no QTL was present as seen in the CTL profile X3.Hydroxypropyl 
  (Fig. \ref{fig:singletraitLOD}), these are the cases in which the loss of variation in the genes 
  underneath the CTL is not causing variation differences in the observed phenotype, but still 
  ...
  the CTL to the phenotype investigated.\\
  ...
  \emph{\sffamily CTL combined with QTL}\\
  Plots of the results of the CTL mapping in Arabidopsis can be found in 
  Figure XX Phenotype to Marker interaction, which is comparable to the QTL mapping 
  (Fig. XX). The additional information obtained by CTL mapping is shown in a heatmap 
  of the Phenotype to Phenotype interaction matrix (figure XX) and the same information 
  but now represented as an interaction network is seen in figure XX and in the CTL plot
  ...
  which shows mappable variation changes, but no CTL colocatizing (Inducer).
  
  \subsection{Example 2. S. Cerevisiae}
  CTL mapping is computationaly expensive (We need 1000+ permutations for each phenotype), 
  ...
  thresholds for CTL mapping were determined for each trait by using 1000 permutations 
  ({\emph Data not shown}).
...
\nopagebreak
  ...
  amounts of heritability explained by individuals loci. CTL mapping however 'borrows' 
  ...
  architecture. Also CTL allows us to better identify gene transcript introducing 
...
%\begin{figure}[ht]
%  \centerline{\includegraphics[height=7cm]{CTL.pdf}}
%  \vspace{-15pt}
%  \caption{{\scriptsize CTL mapping: We start by looking at a single genetic marker and dividing our populaiton into two groups.
%  We start by choosing a phenotype (red), and use its expression to detect the red module (high correlation in gray genotype). 
%  We see that something underlying our current marker in the gray genotype regulates the expression of red (but not blue)}}
%  \vspace{-20pt}
%  \label{fig:CTL}
%\end{figure}

%\begin{figure}[ht]
%  \centerline{\includegraphics[height=6cm]{CTL_n.pdf}}
%  \vspace{-15pt}
%  \caption{{\scriptsize Reconstructed pathway location from gene expression data: Insight into the underlying (hidden) regulatory network 
%  can be obtained when we look at the amount of expression showing the same switch. Depending on the amount of blue expressions 
%  co-affected we can guestimate the location of the red expression inside the pathway}}
%  \vspace{-20pt}
%  \label{fig:CTL_n}
%\end{figure}
...
op <- par(cex=1)
@ 
  \caption{{\scriptsize CTL scores obtained during permutations (A. Thaliana) additional information obtained by CTL mapping, 
  which can be used to reconstruct the underlying genetic architecture. }}
  \label{fig:histPermAra}
\end{figure}
...
op <- par(cex=1)
@
  \caption{{\scriptsize CTL profile (A. Thaliana, X3.Hydroxypropyl) transformed to LOD scores compared to the QTL profile of the same phenotype, We see an 
  additional significant peek for this phenotype (Between Marker 50 to 60) }}
  \label{fig:singletrait}
\end{figure}

\begin{figure}
  \centering
<<singletraitLOD, fig=TRUE, echo=FALSE, height=6.0>>=
op <- par(cex=0.5)
CTLasLOD(ath.result[[1]], ath.result[[1]]$qtl)
op <- par(cex=1)
@ 
  \caption{{\scriptsize CTL profile (A. Thaliana, X3.Hydroxypropyl) transformed to LOD scores. Shown is the likelyhood contribution of each of 
  the 24 metabolites to the CTL profile.}}
  \label{fig:singletraitLOD}
\end{figure}
...
@ 
  \caption{{\scriptsize Phenotypes x Markers matrix (A. Thaliana) this is 'comparable' to a QTL scan on multiple traits.}}
  \label{fig:p2mmatrix}
\end{figure}
...
@
  \caption{{\scriptsize Heatmap showing QTL scores for all 24 metabolities in A. Thaliana.}}
  \label{fig:QTLmatrix}
\end{figure}
...
@ 
  \caption{{\scriptsize Phenotypes x Phenotypes matrix (A. Thaliana) additional information obtained by CTL mapping, 
  which can be used to reconstruct the underlying genetic architecture}}
  \label{fig:p2pmatrix}
\end{figure}
