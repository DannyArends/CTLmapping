...
image.QCLscan <- function(x, QCLpermute, against = c("markers","phenotypes"), do.grid=TRUE, grid.col = "black", verbose = FALSE, ...){
  colorrange <- c("white",gray.colors(10)[10:1])
  if(missing(QCLpermute)){
    if(verbose) stop("argument 'QCLpermute' is missing, with no default\n")
  }else{
    mymatrix <- NULL
    for(p in 1:length(x)){
