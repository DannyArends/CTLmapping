  ...
  for(QCL in QCLscan){
    for(x in 1:ncol(QCL$qcl)){
      ...
      for(id in which(abs(QCL$qcl[,x]) > getPermuteThresholds(QCL)[1])){
        edge_name <- paste(attr(QCL$qcl,"name"),"QCL",colnames(QCL$qcl)[x],sep="\t")
        if(edge_name %in% edge_count[,1]){
          edgeid <- which(edge_count[,1] %in% edge_name)
          if(verbose) cat("Duplicate edge",edgeid,":", edge_name,"\n")
          edge_count[edgeid,2] <- as.numeric(edge_count[edgeid,2])+abs(QCL$l[id,x])
          edge_count[edgeid,3] <- as.numeric(edge_count[edgeid,3])+1
        }else{
          edge_count <- rbind(edge_count,c(edge_name, QCL$l[id,x], 1))
        }
  ...
  for(QCL in QCLscan){
    for(x in 1:ncol(QCL$qcl)){
      ...
      for(id in which(abs(QCL$qcl[,x]) > getPermuteThresholds(QCL)[1])){
        ...
        edge_name2 <- paste(rownames(QCL$qcl)[id],"QCL",attr(QCL$qcl,"name"),sep="\t")
        if(edge_name %in% edge_count[,1] | edge_name2 %in% edge_count[,1]){
          edgeid <- c(which(edge_count[,1] %in% edge_name),which(edge_count[,1] %in% edge_name2))
          if(verbose) cat("Duplicate edge",edgeid,":", edge_name,"\n")
          edge_count[edgeid,2] <- as.numeric(edge_count[edgeid,2])+abs(QCL$l[id,x])
          edge_count[edgeid,3] <- as.numeric(edge_count[edgeid,3])+1
        }else{
