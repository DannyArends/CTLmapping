...
source("../whichGroup.R")
library(ctl)                                            # Load the CTL package
...
phenotypes <- GN111[which(GN111[,"Gene.Symbol"] %in% highImpact[,1]),]
...
if(!file.exists("GN111/QTLs.txt")) {
  ...
  write.table(QTLs, file = "GN111/QTLs.txt", sep="\t", quote=FALSE)
}else{
  QTLs <- read.table("GN111/QTLs.txt")
}

# Map CTL, here we can only use a subset at a time ( I do 100, then close R and continue)
if(!file.exists("GN111/CTLs_p.txt")) {
  CTLs <- matrix(NA, nrow(phenotypes), nrow(genotypes))
  colnames(CTLs) <- rownames(genotypes)

  cat("", file="GN111/CTLs_p.txt")
  cat("", file="GN111/CTLs_int.txt")
  for(x in 1:nrow(phenotypes)){
    ...
    for(y in significant){
      ...
      cat(t(interacts), file="GN111/CTLs_int.txt", append = TRUE)
    }
    cat(paste0(paste0(c(x, scores),collapse="\t"), "\n"), file="GN111/CTLs_p.txt", append = TRUE)
    CTLs[x,] <- scores
    if(x %% 4 == 0) cat(x, "\n")
  }
  write.table(CTLs, file = "GN111/CTLs.txt", sep="\t", quote=FALSE)
}else{
  CTLs <- read.table("GN111/CTLs_p.txt", row.names=1)
}
...
for(x in haveQTL) {
  cat(annotation[x,1], whichGroup(annotation[x,1], highImpact), max(-log10(QTLs[x,])), "\n")
}

for(x in haveCTL) {
  cat(annotation[x,1], whichGroup(annotation[x,1], highImpact), max(CTLs[x,]), "\n")
}
...
interactions <- trim.trailing(readLines("GN111/CTLs_int.txt"))
