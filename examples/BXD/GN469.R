...
### Phenotypes
BXDdata <- read.csv(gzfile("GN469/GN469_MeanDataAnnotated_rev081815.txt.gz"), skip = 33, header = TRUE, sep="\t", colClasses="character")
...
if(!file.exists("GN469/QTLs.txt")) {
  ...
  write.table(QTLs, file = "GN469/QTLs.txt", sep="\t", quote=FALSE)
}else{
  QTLs <- read.table("GN469/QTLs.txt")
}

# Map CTL, here we can only use a subset at a time ( I do 100, then close R and continue)
if(!file.exists("GN469/CTLs_p.txt")) {
  CTLs <- matrix(NA, nrow(phenotypes), nrow(genotypes))
  colnames(CTLs) <- rownames(genotypes)

  cat("", file="GN469/CTLs_p.txt")
  cat("", file="GN469/CTLs_int.txt")
  ...
  write.table(CTLs, file = "GN469/CTLs.txt", sep="\t", quote=FALSE)
}else{
...
interactions <- trim.trailing(readLines("GN469/CTLs_int.txt"))
...
cat(gsub(" ","\t",trim(readLines("GN469/CTLs_int.txt")[uniques])),sep="\n", file="GN469/CTLs_int_all.txt")

# Read in the interaction table and format for cytoscape
interactions <- trim.trailing(readLines("GN469/CTLs_int.txt")[which(substr(readLines("GN469/CTLs_int.txt"),1,1) != " ")])

cat(gsub(" ","\t",trim(readLines("GN469/CTLs_int.txt"))),sep="\n", file="GN469/CTLs_int_all.txt")
...
uniques <- which(!duplicated(apply(apply(cbind(sourc,targe,chr,pos),1,sort),2,paste0,collapse="-")))
cat(gsub(" ","\t",trim(readLines("GN469/CTLs_int.txt")[uniques])),sep="\n", file="GN469/CTLs_int_unique.txt")
