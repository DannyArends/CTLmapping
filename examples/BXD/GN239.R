...
### Phenotypes
BXDdata <- read.csv(gzfile("GN239/GN239_MeanDataAnnotated_rev081815.txt.gz"), skip = 33, header = TRUE, sep="\t", colClasses="character")
...
if(!file.exists("GN239/QTLs.txt")) {
  ...
  write.table(QTLs, file = "GN239/QTLs.txt", sep="\t", quote=FALSE)
}else{
  QTLs <- read.table("GN239/QTLs.txt")
}

# Map CTL, here we can only use a subset at a time ( I do 100, then close R and continue)
if(!file.exists("GN239/CTLs_p.txt")) {
  CTLs <- matrix(NA, nrow(phenotypes), nrow(genotypes))
  colnames(CTLs) <- rownames(genotypes)

  cat("", file="GN239/CTLs_p.txt")
  cat("", file="GN239/CTLs_int.txt")
  for(x in 1:nrow(phenotypes)){
    ...
    for(y in significant){
      ...
      cat(t(interacts), file="GN239/CTLs_int.txt", append = TRUE)
    }
    cat(paste0(paste0(c(x, scores),collapse="\t"), "\n"), file="GN239/CTLs_p.txt", append = TRUE)
    CTLs[x,] <- scores
    if(x %% 4 == 0) cat(x, "\n")
  }
  write.table(CTLs, file = "GN239/CTLs.txt", sep="\t", quote=FALSE)
}else{
  CTLs <- read.table("GN239/CTLs_p.txt", row.names=1)
}
...
interactions <- trim.trailing(readLines("GN239/CTLs_int.txt"))
...
cat(gsub(" ","\t",trim(readLines("GN239/CTLs_int.txt")[uniques])),sep="\n", file="GN239/CTLs_int_all.txt")

# Read in the interaction table and format for cytoscape
interactions <- trim.trailing(readLines("GN239/CTLs_int.txt")[which(substr(readLines("GN239/CTLs_int.txt"),1,1) != " ")])

cat(gsub(" ","\t",trim(readLines("GN239/CTLs_int.txt"))),sep="\n", file="GN239/CTLs_int_all.txt")
...
uniques <- which(!duplicated(apply(apply(cbind(sourc,targe,chr,pos),1,sort),2,paste0,collapse="-")))
cat(gsub(" ","\t",trim(readLines("GN239/CTLs_int.txt")[uniques])),sep="\n", file="GN239/CTLs_int_unique.txt")
