...
ctleffects <- function(genotypes, phenotypes, phe = 1, nthreads = 1, verbose = TRUE) {
  ...
  r <- parLapply(cl, split(genotypes, rep(1:ncol(genotypes), each = nrow(genotypes))), 
    function(marker, phenotypes, n.markers) {
      require(ctl)
      mgt <- unique(marker)
    ...
    , phenotypes, ncol(genotypes))
  names(r) <- colnames(genotypes)
  stopCluster(cl)
  return(r)
}

#split(genotypes, rep(1:ncol(genotypes), each = nrow(genotypes)))
