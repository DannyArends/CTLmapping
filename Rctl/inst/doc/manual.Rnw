...
\textbf{\sffamily Abstract}: Tutorial for the Correlated Trait Loci (CTL) mapping to reconstruct
genetic regulatory networks using the R package 'ctl' and the mapctl commandline tool. This 
tutorial is targetted at people with a basic understanding of genetics and would like to analyse
there inbred cross or natural population. Main focus will be explaining the functionality of the 
software, and a how-to on loading datasets via Excel, tab delimited files (.csv) and R/qtl. In 
...
The phenotypes.csv file containing the genotype matrix is stored individuals x genetic marker:
...
conditions <- c(rep(1,51),rep(2,49),rep(2,62))
