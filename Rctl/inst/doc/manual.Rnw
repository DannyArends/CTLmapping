...
\textbf{\sffamily Abstract}: Introduction into quanitative correlation locus (CTL) mapping using 
the R package 'ctl'
\vfill
...
phenotypes <- read.csv("phenotypes.csv",row.names=1,col.names=TRUE)
ctls <- CTLscan(genotypes, phenotypes)
@
