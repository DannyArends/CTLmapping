...
analyse there inbred cross or natural population. Main focus will be explaining the functionality 
of the software, and a how-to on loading datasets via Excel, tab delimited files (.csv) and 
R/qtl. In the methodology and background only a short introduction in CTL mapping theory is 
given.
