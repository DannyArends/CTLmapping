...
import ctl.core.stats.basic, ctl.core.ctl.utils;
...
double[][] tolod(double[][] scores, double[] permlist, size_t[] significant, string[] phenonames, bool verbose = true){
  ...
  writefln("Above threshold: %s traits ( %s )", m.length, get(phenonames,significant));
  if(verbose) writefln("LOD transformation: (%s msecs)",(Clock.currTime()-stime).total!"msecs"());  
  return m;
}
...
double getCutoff(in double[] permlist, double minlod){
  ...
  writefln("FDR %s|%s, %s, %s|%s",pow(10,-estimate), fdr, critval, getEstimate(permlist,critval), minlod);
  return critval;
}
